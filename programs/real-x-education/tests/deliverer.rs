...
fn reclaim_full_restores_allocation() {
    ...
    ok(
        &mut w.svm,
        reclaim_sponsorship_ix(&sponsor.pubkey(), 0, 0, 5),
        &sponsor,
        &[&sponsor],
    );
    ...
    assert_eq!(
        balance(&w.svm, &usdc_mint(), &sponsor.pubkey()) - spon_before,
        5 * PER_TOKEN
    );
...
fn unregister_strips_role_and_blocks_reregister() {
    ...
    assert!(closed(
        &w.svm,
        &role_pda(&lecturer.pubkey(), Role::ModuleDeliverer)
    ));
    err(
        &mut w.svm,
        register_deliverer_ix(&lecturer.pubkey()),
    ...
    ok(
        &mut w.svm,
        roles_assign_ix(&admin.pubkey(), &lecturer.pubkey(), Role::ModuleDeliverer),
        &admin,
        &[&admin],
    );
    ok(
...
fn unregister_recovers_deposit_after_role_gone() {
    ...
    ok(
        &mut w.svm,
        roles_remove_ix(&admin.pubkey(), &lecturer.pubkey(), Role::ModuleDeliverer),
        &admin,
        &[&admin],
    );
    assert!(closed(
    ...
    assert_eq!(
        balance(&w.svm, &xcav_mint(), &lecturer.pubkey()) - before,
        DELIVERER_DEPOSIT
    );
