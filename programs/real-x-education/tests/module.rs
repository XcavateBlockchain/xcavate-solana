...
fn sponsor_module_works() {
    ...
    assert_eq!(
        before - balance(&w.svm, &usdc_mint(), &sponsor.pubkey()),
        5 * PER_TOKEN
    );
...
fn close_sponsorship_blocked_while_booking_active() {
    ...
    assert!(w
        .svm
        .get_account(&sponsor_escrow_pda(0, 0))
...
fn close_sponsorship_after_reclaim_works() {
    ...
    assert!(w
        .svm
        .get_account(&sponsorship_pda(0, 0))
        .map(|a| a.data.is_empty())
        .unwrap_or(true));
    assert!(w
        .svm
...
fn create_module_fails() {
    ...
    err(
        &mut w.svm,
        create_module_ix(&creator.pubkey(), 1, 0, 0),
        &creator,
        &[&creator],
        "AmountCannotBeZero",
    );
    ...
    err(
        &mut w.svm,
        create_module_ix(&creator.pubkey(), 1, 0, 1_001),
        &creator,
        &[&creator],
        "TooManyTokens",
    );
...
fn sponsor_module_multi_asset_works() {
    ...
    ok(
        &mut w.svm,
        sponsor_ix(&sponsor.pubkey(), 0, 0, 4),
        &sponsor,
        &[&sponsor],
    );
    ...
    ok(
        &mut w.svm,
        sponsor_asset_ix(&sponsor.pubkey(), 0, 1, 3, &gbp_mint()),
        &sponsor,
        &[&sponsor],
    );
    ...
    assert_eq!(
        gbp_before - balance(&w.svm, &gbp_mint(), &sponsor.pubkey()),
        3 * PER_TOKEN_GBP
    );
    assert_eq!(
        spl_amount(&w.svm, &sponsor_escrow_pda(0, 1)),
        3 * PER_TOKEN_GBP
    );
...
fn sponsor_module_fails() {
    ...
    err(
        &mut w.svm,
        sponsor_ix(&sponsor.pubkey(), 0, 0, 0),
        &sponsor,
        &[&sponsor],
        "AmountCannotBeZero",
    );
    ...
    err(
        &mut w.svm,
        sponsor_ix(&sponsor.pubkey(), 0, 0, 11),
        &sponsor,
        &[&sponsor],
...
fn burn_unsponsored_fails() {
    ...
    ok(
        &mut w.svm,
        sponsor_ix(&sponsor.pubkey(), 0, 0, 8),
        &sponsor,
        &[&sponsor],
    );
...
fn remove_module_works() {
    ...
    assert_eq!(
        balance(&w.svm, &xcav_mint(), &creator.pubkey()) - before,
        MODULE_DEPOSIT
    );
